...
| 模型 | 输入尺寸 | 类别数 | 原始浮点模型参数量 | 原始浮点模型计算量 |
|---|---:|---:|---:|---:|
| YOLO11m Detect | 640×640 | 80 | 20.1 M | 68.0 B FLOPs |
...
## CPU Latency（Single Core）补测

| 指标 | 均值 | P50 | P95 | P99 | 最小值 | 最大值 |
|---|---:|---:|---:|---:|---:|---:|
| CPU后处理延迟 | 1.744 ms | 1.655 ms | 2.219 ms | 2.601 ms | 1.306 ms | 80.933 ms |
...
该指标只统计阈值过滤、DFL框解码、NMS和坐标恢复，不包含图片解码、Letterbox预处理、BPU推理和COCO指标计算。
